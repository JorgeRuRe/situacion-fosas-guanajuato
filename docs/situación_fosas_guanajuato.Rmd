...
```{r}
# Mapear fosas prensa
guanajuato_mapa_prensa %>%
  filter(CVE_ENT %in% c("11")) %>% 
  ggplot(aes(fill = total_fosas)) +
  # cambia el color y el grosor de las líneas
  geom_sf(colour = "grey20", size = 0.07) +
  # agrega títulos
  labs(title = "Fosas clandestinas observadas por prensa",
       subtitle = "2007-2017",
       caption = "PDHIBERO") +
  # cambia el color de relleno y el título de la leyenda
  scale_fill_gradient("Total de fosas", high = "#EF5351", low = "#F4E8C1") +
